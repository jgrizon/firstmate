...
Preserve its uncommitted changes and commits, keep the same task identity, and relaunch the recorded harness in that existing worktree via `bin/fm-control.sh <id> relaunch --note '<progress so far>'` with the same brief plus a concise progress note (recreating the backend endpoint if missing).
