...
| Trust dialog | `Do you trust the contents of this project?` with `Yes, I trust this folder` preselected, cleared by one Enter. `--dangerously-skip-permissions` does not suppress it and agy exposes no `--trust` flag, so `fm-spawn` accepts it in the pane after launch. The decision persists in `~/.gemini/antigravity-cli/settings.json` under `trustedWorkspaces` and does NOT cascade to subdirectories, so every fresh task worktree hits it. The post-launch wait stops early on exactly one signal: agy's OWN footer, either token, on the live rows of the capture. No composer verdict is accepted, because none of them proves agy is up - `empty` is reachable through the classifier's bare-glyph path with no identity probe, so a pane whose agy died and whose shell redrew a bare `❯` prompt reads `empty` on its own, and `pending` is what that same row reads as while it still carries the launch command. The footer is the one thing on that screen a shell cannot emit. Either token counts and the idle one is not required: `-i` means agy is mid-turn (`esc to cancel`) for the whole post-launch window, so an idle-only signal would shut the gate exactly when it is needed and cost every spawn its full poll budget. |
...
There is a THIRD artifact teardown does not retire, and it is accepted rather than fixed: the worktree path agy records in its own `~/.gemini/antigravity-cli/settings.json` under `trustedWorkspaces` when the folder-trust dialog is accepted.
Firstmate never writes that store, which is the same high-blast-radius rule that keeps it out of grok's, so teardown has nothing it may safely edit there.
The practical bound is the worktree pool: slots are reused, so accepted paths repeat rather than accumulating one per task.
Reaching into another vendor's config store to prune it is out of scope and would be fragile across their releases.
