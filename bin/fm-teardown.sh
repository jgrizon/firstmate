...
# tasks, then print a backlog-refresh reminder for ship, scout, and lab teardowns
...
# Scout and lab tasks (kind=scout or kind=lab in meta) carve out of that check: their
# deliverable is the report at data/<task-id>/report.md rather than landed code.
# Teardown proceeds only once the report exists and the shared unresolved-decision
# completion gate verifies its captain-held inventory.
# A scout worktree is scratch and returned to the pool; a lab directory at
# data/<task-id>/lab/ is retired by moving it to data/.labs-retired/<task-id>/ rather
# than deleting it.
...
#   --force skips ordinary-task dirty and landed-work checks, skips scout/lab report
...
# The harnesses whose turn-end hook is GLOBAL and gated by a private registry
# token, so teardown must retire the registry entry as well as the task's own
# state. Where each entry lives is owned by bin/fm-control-lib.sh, so teardown
# and the control plane's relaunch retire the same artifact rather than each
# carrying its own copy of the path.
FM_TEARDOWN_TURNEND_TOKEN_HARNESSES='grok kimi agy'

# Remove every worktree-resident hook and pointer file any harness may leave
# behind. Done unconditionally rather than per recorded harness, because a
# pooled worktree is handed to the NEXT task and a leftover pointer would fire
# this dead task's signals for its successor.
remove_worktree_hook_files() {  # <worktree>
  rm -f "$1/.claude/settings.local.json" \
    "$1/.opencode/plugins/fm-turn-end.js" \
    "$1/.opencode/plugins/fm-busy-state.js" \
    "$1/.fm-grok-turnend" "$1/.fm-kimi-turnend" "$1/.fm-agy-turnend"
}
...
# Retire every global turn-end registry entry this task could have minted. The
# task's recorded harness is not consulted: a relaunch can have switched
# harness, and an entry left behind would keep a retired incarnation armed.
remove_all_turnend_auth() {  # <state-dir> <id>
  local harness
  for harness in $FM_TEARDOWN_TURNEND_TOKEN_HARNESSES; do
    remove_turnend_auth "$harness" "$1" "$2" || return 1
  done
}
...
if { [ "$KIND" = scout ] || [ "$KIND" = lab ]; } && [ "$FORCE" != "--force" ]; then
...
if [ "$BACKEND" = orca ] && [ "$KIND" != scout ] && [ "$KIND" != lab ] && [ "$KIND" != secondmate ] && [ "$FORCE" != "--force" ]; then
...
if [ "$BACKEND" = orca ] && [ "$KIND" != secondmate ]; then
  ...
  wt_canon=$(cd "$WT" 2>/dev/null && pwd -P || printf '%s' "$WT")
  expected_canon=$(cd "$LAB_EXPECTED" 2>/dev/null && pwd -P || printf '%s' "$LAB_EXPECTED")
  if [ "$wt_canon" != "$expected_canon" ]; then
    echo "REFUSED: lab task $ID recorded worktree '$WT' does not match expected '$LAB_EXPECTED'; refusing teardown" >&2
    exit 1
  fi
  if [ -d "$WT" ]; then
    remove_worktree_hook_files "$WT"
    LABS_RETIRED="$DATA/.labs-retired"
    mkdir -p "$LABS_RETIRED"
    RETIRED_DEST="$LABS_RETIRED/$ID"
    rm -rf "$RETIRED_DEST" 2>/dev/null || true
    mv "$WT" "$RETIRED_DEST"
    echo "teardown: retired lab $ID to $RETIRED_DEST"
  fi
...
META_LOCK_HELD=0
if [ "$KIND" != scout ] && [ "$KIND" != lab ] && [ "$KIND" != secondmate ] && [ "$MODE" != local-only ]; then
